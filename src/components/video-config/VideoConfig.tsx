...
type Props = { value: VideoConfiguration; directory: DirectoryHandleLike | null; imageModeAvailable: boolean; disabled: boolean; onChange(value: VideoConfiguration): void; onChooseDirectory(): void; onGenerate(): void };

export function VideoConfig({ value, directory, imageModeAvailable, disabled, onChange, onChooseDirectory, onGenerate }: Props) {
  return <section className="panel">
    ...
    <div className="config-grid">
      {imageModeAvailable && <label className="wide"><span>Cách dựng từ ảnh</span><select value={value.renderMode} onChange={(event) => {
        const renderMode = event.target.value as VideoConfiguration["renderMode"];
        onChange({ ...value, renderMode, duration: renderMode === "ai-product" ? 15 : value.duration });
      }}><option value="smart-motion">Smart Motion 2.5D · dựng local</option><option value="ai-product">AI Product Video · Runway</option></select></label>}
      ...
      <label><span>Thời lượng</span><select value={value.duration} disabled={imageModeAvailable && value.renderMode === "ai-product"} onChange={(e) => onChange({ ...value, duration: Number(e.target.value) as 15|20|30 })}><option value="15">15 giây</option><option value="20">20 giây</option><option value="30">30 giây</option></select></label>
    ...
    <p className="ai-disclosure">ⓘ Giọng đọc được tạo bởi AI, không phải giọng người thật.{imageModeAvailable && value.renderMode === "ai-product" ? " Ảnh nén sẽ được gửi tới Runway và mỗi video có thể phát sinh credits." : " Smart Motion 2.5D được dựng ngay trong trình duyệt."}</p>
